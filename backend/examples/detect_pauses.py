...
def main():
    ...
    if args.filter_min or args.filter_max:
        ...
        pauses = filter_pauses_by_duration(
            pauses,
            min_duration=args.filter_min,
            max_duration=args.filter_max
        )
        print(f"After filtering: {len(pauses)} pause(s) (removed {original_count - len(pauses)})\n")
    
    # Merge nearby pauses if requested
    if args.merge_gap:
        original_count = len(pauses)
        pauses = merge_nearby_pauses(pauses, max_gap=args.merge_gap)
        print(f"After merging: {len(pauses)} pause(s) (merged {original_count - len(pauses)})\n")
